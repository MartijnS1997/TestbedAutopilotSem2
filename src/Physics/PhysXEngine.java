...
import Autopilot.AutopilotTurn;
...
public class PhysXEngine {
	     ...
	     * @param deltaTime the time step for the next state (time between simulation steps)
...
//	        };
	        ...
	        PhysicsEngineState physicsEngineState = new PhysicsEngineState() {
	            ...
	            public float getFrontTyreDelta() {
	                return chassisPhysX.getFrontTyre().calcRadiusDelta(orientation, position);
	            }

	            @Override
	            public float getRearLeftTyreDelta() {
	                return chassisPhysX.getRearLeftTyre().calcRadiusDelta(orientation, position);
	            }

	            @Override
	            public float getRearRightTyreDelta() {
	                return chassisPhysX.getRearRightTyre().calcRadiusDelta(orientation, position);
	            }
	        };

	       
//	        System.out.println("current orientation: " + physicsEngineState.getOrientation().scalarMult((float) (180/PI)));
//	        System.out.println();
...
//	        return heading.matrixProduct(pitch).matrixProduct(roll);
	     ...
	     * @param deltaTime the time that has passed between two following simulation steps
	    ...
	    /**
	     * Calculates all the sum of all the forces exerted on the drone while it is airborne
	     * for the given inclinations in the outputs (the result is provided in the world axis system)
	     * @param orientation the orientation of the drone
	     * @param rotation the rotation of the drone
	     * @param velocity the velocity of the drone
	     * @param outputs the outputs to extract the inclination for the wings from
	     * @return the sum of the lift generated by the drone and gravity (thrust is not added because it is variable)
	     *         given in the world axis system
	    ...
	    public Vector getAirborneForces(Vector orientation, Vector rotation, Vector velocity, AutopilotOutputs outputs){
	        ...
	        float horizontalInclination = outputs.getHorStabInclination();
	        float verticalInclination = outputs.getVerStabInclination();

	        //set the orientations to the given outputs
	        this.getMainLeft().setWingInclination(leftWingInclination);
	        this.getMainRight().setWingInclination(rightWingInclination);
	        this.getHorizontalStabilizer().setWingInclination(horizontalInclination);
	        this.getVerticalStabilizer().setWingInclination(verticalInclination);
	        ...
	        //calculate the force on all the wings
	        for (int index = 0; index != nbOfWings; index++) {
	            liftVectors[index] = wingArray[index].getLift(orientation, rotation, velocity);
	        }

	        //sum to get the total lift
	        Vector totalLift = Vector.sumVectorArray(liftVectors);
	        ...
	        // get the gravitational force exerted on the drone
	        Vector gravity = new Vector(0, gravityYComp, 0);
	        // sum it all up & return
	        return gravity.vectorSum(totalLift);
	    }
...
//	        System.out.println("Lift Horizontal stabilizer: " + forceVectorsDrone[2]);
//	        System.out.println("Lift Vertical stabilizer: " + forceVectorsDrone[3]);
	    ...
	    public class PhysXOptimisations {
	        ...
	        private AutopilotOutputs DUMMY_OUTPUTS = new AutopilotOutputs() {
	            @Override
	            public float getThrust() {
	                return 0;
	            }
