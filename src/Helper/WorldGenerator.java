...
//		float z = -40f;
    ...
    /**
     * Creates a random path based on the base vector and the previously determined variables
     * returns random path
     */
    public void createPath(Vector baseVector, String realPathDir, String approxPathDir){
        int n = getNbOfCubes();
...
//		setErroredPositions(errorPosition);
        ...
        //save the real path
        Path realPath = convertToPath(positionsAdded);
        writePath(realPath, realPathDir);

        //randomize the path created
        Path randomizedPath= pathRandomizer(positionsAdded);
        writePath(randomizedPath, approxPathDir);
    }


    private void writePath(Path path, String fileName){
        try {
            PrintWriter writer = new PrintWriter(fileName, "UTF-8");

            float[] xPos = path.getX();
            float[] yPos = path.getY();
            float[] zPos = path.getZ();

            int size = xPos.length;

            for (int index = 0; index != size; index++) {
                writer.write(Float.toString(xPos[index]) + " ");
                writer.write(Float.toString(yPos[index]) + " ");
                writer.write(Float.toString(zPos[index]) + " ");
                writer.write("\r\n");
            }
            writer.close();
        }catch (Exception e){
     ...
     */
    private Path pathRandomizer(List<Vector> pathPositions){
